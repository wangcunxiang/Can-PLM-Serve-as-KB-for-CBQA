...
def preprocess_data(data):
    ...
    if args.preprocess_inputs:
        ...
        target_text = tokenizer.encode(
            target_text + " </s>", max_length=args.max_seq_length, truncation=True, pad_to_max_length=True, return_tensors="pt"
        )
    else:
        input_text = tokenizer.encode(
            prefix + input_text, max_length=args.max_seq_length, truncation=True, pad_to_max_length=True, return_tensors="pt"
        )

        target_text = tokenizer.encode(
            target_text, max_length=args.max_seq_length, truncation=True, pad_to_max_length=True, return_tensors="pt"
        )
